namespace MyONez.Base.Screens
{
    ...
    [SampleScene("UI Scene", "Scene UI from GeonBitUi example project")]
    public class UIScene : BaseScene
    {
        public UIScene()
        {
            ...
            var uiComponent = ui.AddComponent<UIComponent>();
        ...
        /// </summary>        
        private UserInterface InitExamplesAndUI()
        {
            ...
            userInterface.OnClick = (entity) => { eventsLog.AddItem("Click: " + entity.GetType().Name); eventsLog.scrollToEnd(); };
            userInterface.OnRightClick = (entity) => { eventsLog.AddItem("RightClick: " + entity.GetType().Name); eventsLog.scrollToEnd(); };
            userInterface.OnMouseDown = (entity) => { eventsLog.AddItem("MouseDown: " + entity.GetType().Name); eventsLog.scrollToEnd(); };
            userInterface.OnRightMouseDown = (entity) => { eventsLog.AddItem("RightMouseDown: " + entity.GetType().Name); eventsLog.scrollToEnd(); };
            userInterface.OnMouseEnter = (entity) => { eventsLog.AddItem("MouseEnter: " + entity.GetType().Name); eventsLog.scrollToEnd(); };
            userInterface.OnMouseLeave = (entity) => { eventsLog.AddItem("MouseLeave: " + entity.GetType().Name); eventsLog.scrollToEnd(); };
            userInterface.OnMouseReleased = (entity) => { eventsLog.AddItem("MouseReleased: " + entity.GetType().Name); eventsLog.scrollToEnd(); };
            userInterface.OnMouseWheelScroll = (entity) => { eventsLog.AddItem("Scroll: " + entity.GetType().Name); eventsLog.scrollToEnd(); };
            userInterface.OnStartDrag = (entity) => { eventsLog.AddItem("StartDrag: " + entity.GetType().Name); eventsLog.scrollToEnd(); };
            userInterface.OnStopDrag = (entity) => { eventsLog.AddItem("StopDrag: " + entity.GetType().Name); eventsLog.scrollToEnd(); };
            userInterface.OnFocusChange = (entity) => { eventsLog.AddItem("FocusChange: " + entity.GetType().Name); eventsLog.scrollToEnd(); };
            userInterface.OnValueChange = (entity) => { if (entity.Parent == eventsLog) { return; } eventsLog.AddItem("ValueChanged: " + entity.GetType().Name); eventsLog.scrollToEnd(); };
            ...
            userInterface.WhileDragging = (entity) => { eventsNow.AddItem("Dragging: " + entity.GetType().Name); eventsNow.scrollToEnd(); };
            userInterface.WhileMouseDown = (entity) => { eventsNow.AddItem("MouseDown: " + entity.GetType().Name); eventsNow.scrollToEnd(); };
            userInterface.WhileMouseHover = (entity) => { eventsNow.AddItem("MouseHover: " + entity.GetType().Name); eventsNow.scrollToEnd(); };
            ...
            {
                if (userInterface.RenderTargetTransformMatrix == null)
                {
                    userInterface.RenderTargetTransformMatrix = Matrix.CreateScale(0.6f) *
...
The most common anchors are 'Auto' and 'AutoInline', which will place entities one after another automatically.",
                ...
                // example: animators
                {
                    ...
                    // float up-down
                    {
                        ...
                        entity.AttachAnimator(new FloatUpDownAnimator());
                    }
                    ...
                    // wave animation
                    {
                        ...
                        entity.AttachAnimator(new TextWaveAnimator());
                    }
                    ...
                    // fade out
                    {
                        ...
                        var animator = entity.AttachAnimator(new FadeOutAnimator() { Enabled = false });
                        entity.OnClick += (ent) =>
                        {
                    ...
                    // type writer animator
                    {
                        ...
                        var animator = entity.AttachAnimator(new TypeWriterAnimator()
                        {
...
Maybe something interesting in tab3?"));
                ...
                // example: file menu
                {
                    ...
                    layout.AddItemToMenu("Display", "Reset Zoom", () => { userInterface.GlobalScale = 1f; });
                    ...
                    fileMenu = SimpleFileMenu.Create(layout, PanelSkin.Fancy);
                    fileMenu.Anchor = Anchor.Auto;
                    panel.AddChild(fileMenu);
                }
                ...
                // example: Cursors
                {
                    ...
                    // default cursor show
                    {
                        ...
                        btn.OnMouseEnter = (entity) => { userInterface.SetCursor(CursorType.Default); };
                        btn.OnMouseLeave = (entity) => { userInterface.SetCursor(CursorType.Default); };
                        panel.AddChild(btn);
                    }
                    ...
                    // pointer cursor show
                    {
                        ...
                        btn.OnMouseEnter = (entity) => { userInterface.SetCursor(CursorType.Pointer); };
                        btn.OnMouseLeave = (entity) => { userInterface.SetCursor(CursorType.Default); };
                        panel.AddChild(btn);
                    }
                    ...
                    // ibeam cursor show
                    {
                        ...
                        btn.OnMouseEnter = (entity) => { userInterface.SetCursor(CursorType.IBeam); };
                        btn.OnMouseLeave = (entity) => { userInterface.SetCursor(CursorType.Default); };
                        panel.AddChild(btn);
                    }
                    ...
                    panel.AddChild(new Paragraph("And as always, you can also set your own custom cursor:"));
                    {
                        ...
                        btn.OnMouseEnter = (entity) => { userInterface.SetCursor(Content.Load<Texture2D>("example/cursor"), 40); };
                        btn.OnMouseLeave = (entity) => { userInterface.SetCursor(CursorType.Default); };
                        panel.AddChild(btn);
                    }
...
Click on 'Next' to see the character creation demo."));
                ...
                // example: character build page - final
                {
                    ...
                    foreach (Color baseColor in colors)
                    {
                        for (int i = 0; i < 9; ++i)
                        {
                            Color color = baseColor * (1.0f - (i * 2 / 16.0f));
                            color.A = 255;
